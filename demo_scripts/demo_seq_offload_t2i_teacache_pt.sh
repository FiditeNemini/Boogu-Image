...
pretrained_pipeline_name_or_path="Boogu/Boogu-Image-0.0-Base-exp-pt"
...
experiment_name=demo_seq_offload_t2i_teacache_pt
use_prompt_tuning=True
enable_sequential_cpu_offload_flag=True
enable_teacache_for_all_layers=True
...
# case_name=t2i_oldman_en_seq_offload_pt_teacache_all_layers_${resolution}_steps${num_inference_steps}_tg${text_guidance_scale}_ig${image_guidance_scale}_seed${seed}
case_name=t2i_oldman_zh_seq_offload_pt_teacache_all_layers_${resolution}_steps${num_inference_steps}_tg${text_guidance_scale}_ig${image_guidance_scale}_seed${seed}
...
python inference.py \
    ...
    --enable_sequential_cpu_offload_flag $enable_sequential_cpu_offload_flag \
    --use_prompt_tuning $use_prompt_tuning \
    --enable_teacache_for_all_layers $enable_teacache_for_all_layers \
    --height $height \
    --width $width \
